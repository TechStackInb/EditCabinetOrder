...
// data — either the user has selected a thermofoil (thermofoilId), or it was
...
const isRemovable = (row, total) =>
  total > 1 || Boolean(row.thermofoilId) || Boolean(row._lineItemId);
...
const ThermofoilCard = ({
  ...
  }, [thermofoils, orderColors, showAllColors]);
      ...
      <Flex direction="column" gap="medium">
        ...
        {row.promptColorMismatch && (
          <Alert title="Color does not match" variant="warning">
            The color of this thermofoil ({row.color || '—'}) does NOT match any
            of your selected colors ({orderColors.join(', ') || '—'}). Are you
            sure you want to select this thermofoil?
            <Flex direction="row" gap="small">
              <Button
                variant="primary"
                size="sm"
    ...
    <Flex direction="column" gap="medium">
      ...
      <Flex direction="row" align="center" gap="small">
        ...
        <Text variant="microcopy">
          Show ALL color options (default: only thermofoils matching selected
